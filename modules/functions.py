...
async def check_squad_composition(ctx, squad, role):
    ...
    '''
    if there is no composition, return True
    '''
    ...
    '''
    If there is a composition, check if role valid
    '''
    if role not in squad.composition:
        await handlers.InvalidRole()

    if squad.composition[role] >= 1:
        return True
    else:
        await handlers.RoleCapacity()
        return False
...
class InvalidRole(handlers.Error):

    def __init__(self, ctx, error):

        super().__init__(self, ctx, error)
        self.embed.add_field(name="Exception",
                value="functions.InvalidRole",
                inline=False)

class RoleCapacity(handlers.Error):

    def __init__(self, ctx, error):

        super().__init__(self, ctx, error)
        self.embed.add_field(name="Exception",
                value="functions.RoleCapacity",
                inline=False)
